...
| Path | Purpose |
| --- | --- |
| `hdl_sources/` | Main Verilog source files and constraints |
| `hdl_sources/top.v` | Top-level module connecting clocks, camera, BRAM, filters, and VGA |
| `hdl_sources/constraints.xdc` | Basys 3 pin constraints |
| `sim/` | Cocotb module-level test benches |
| `sim_build/` | Generated simulator output and wave dumps |
| `report/` | Final report draft, template, example, and generated diagram |
| `project-description/` | Project prompt, Vivado project file, and reference PDFs |
| `ov7670_vga_system.gen/` | Generated Vivado IP sources |
| `ov7670_vga_system.runs/` | Vivado synthesis/implementation outputs |
| `PROJECT_GUIDE.md` | Longer explanation of the design and modules |
...
The Vivado project file is stored under `project-description/`:

```sh
vivado project-description/ov7670_vga_system.xpr
```

If Vivado reports missing source paths after moving files, add the files from `hdl_sources/` manually and ensure the generated IP directories under `ov7670_vga_system.gen/` and `ov7670_vga_system.ip_user_files/` are available.

The generated bitstream from the existing implementation run is:

```text
ov7670_vga_system.runs/impl_1/top.bit
```
...
The first Markdown report draft is:

```text
report/Final_Report.md
```

The report diagram is available as:

```text
report/images/overall_design_block_diagram.png
report/images/overall_design_block_diagram.svg
```
...
- OV7670 camera pins: assigned in `hdl_sources/constraints.xdc`